...
void wifiSetup(bool onDemand = false) {
  ...
  String fp_info =
      "<p>Run <pre>echo | openssl s_client -connect MYMQTTSERVER:8883 | "
      "openssl x509 -fingerprint -noout</pre> to get your TLS fingerprint.</p>";
  ...
  WiFiManagerParameter custom_mqtt_user("user", "MQTT Username", mqtt_user,
                                        128);
  ...
  WiFiManagerParameter custom_mqtt_topic("topic", "MQTT Topic", mqtt_topic,
                                         128);
  WiFiManagerParameter custom_mqtt_fingerprint(
      "fingerprint", "MQTT Fingerprint", mqtt_fingerprint, 128);
